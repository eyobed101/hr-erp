...
const handleDelete = async (attendanceId) => {
  try {
    ...
    // Remove from frontend state if needed
    setAttendance(null);
     fetchAttendanceByDate();
  } catch (err) {
        ...
        {attendances.map((att) => (
          <tr key={att.id} className="border-t hover:bg-gray-50">
            ...
            <td className="px-4 py-2">
              {att.clock_in ? new Date(att.clock_in).toISOString().slice(0, 19) : "-"}
            </td>
            <td className="px-4 py-2">
              {att.clock_out ? new Date(att.clock_out).toISOString().slice(0, 19) : "-"}
            </td>
